        ...
        if layerconf:
            ...
            for c in collections:
                ...
                if compat and not (compat & layerseries):
                    bb.fatal("Layer %s is not compatible with the core layer which only supports these series: %s (layer is compatible with %s) %s"
                              % (c, " ".join(layerseries), " ".join(compat), compat))
                elif not compat and not data.getVar("BB_WORKERCONTEXT"):
                    bb.warn("Layer %s should set LAYERSERIES_COMPAT_%s in its conf/layer.conf file to list the core layer names it is compatible with %s %s." % (c, c, " ".join(compat), compat))
