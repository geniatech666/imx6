...
extern void read_dev_info_from_mmc();
extern unsigned char mac_addrs[50];
/**
...
static int eth_post_probe(struct udevice *dev)
{
	...
	} else if (is_valid_ethaddr(pdata->enetaddr) || !is_zero_ethaddr(mac_addrs)) {
		eth_env_set_enetaddr_by_index("eth", dev->seq, pdata->enetaddr);
		printf("\nWarning: %s using MAC address from ROM :mac %pM\n",
		       dev->name,pdata->enetaddr);
