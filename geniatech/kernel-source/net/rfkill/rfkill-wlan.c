...
extern void mmc_pwrseq_post_power_on(struct mmc_host *host);
extern void mmc_pwrseq_pre_power_on(struct mmc_host *host);
...
int rockchip_wifi_power(int on)
{
	...
	if (on && primary_sdio_host){
		LOG(" %s: %d --->on\n", __func__, on);
		mmc_pwrseq_pre_power_on(primary_sdio_host);
                mmc_pwrseq_post_power_on(primary_sdio_host);
	}
...
u8 wifi_custom_mac_addr[6] = {0x11,0x22,0x33,0x44,0x55,0x66};
#if 0
