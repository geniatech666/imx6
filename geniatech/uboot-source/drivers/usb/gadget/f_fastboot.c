...
void read_dev_info_from_mmc(void )
{
        struct dev_info *info = NULL;
        int ret =0,i=0;
        size_t out_num_read;
	int read_size = 512;
	char partition[8][10]={"mmcsdc2","mmcsdb2"};
        info = memalign(PAGE_SIZE, ROUNDUP(BOOT_IMG_MAX_PAGE_SIZE, PAGE_SIZE));
        if(info == NULL)
        {
                printf("Failed to allocate memory for device info struct\n");
		return;
        }
	for(i=0;i<8;i++)
	{
		ret = read_from_partition_multi(partition[i],0,read_size, (void *)info,&out_num_read);
		if(!ret)
			break;
	}
	printf("page_size:%d,out_num_read:%d \n",read_size,out_num_read);
       // ret = mmc_read(ptn, (void *)info, page_size);
        if (ret)
        {
        ...
        printf("nfo->magic:%s\n",info->magic);
        printf("nfo->board_sn:%s\n",info->board_sn);
        printf("nfo->mac_addr:%s\n",info->mac_addr);
        printf("info->flash_lock:0x%x\n",info->flash_lock);

	sprintf(mac_addrs,"%s",info->mac_addr);
	printf("mac_addrs:%s\n",mac_addrs);
	free (info);
}
...
void fastboot_run_bootmode(void)
{
	...
	default:
		...
		for(i=0;i<g_pcount;i++ ){
			//printf("-----%d %s \n",i, g_ptable[i].name);
			ptentry = fastboot_flash_find_ptn(g_ptable[i].name);
			printf(" partion name:  %s,  %d  %d \n", ptentry->name,ptentry->start,ptentry->length);
            if(i == (g_pcount - 2)){
                memcpy(g_partition, ptentry->name, sizeof(ptentry->name));
            }
		}
		//read the params partition
		read_dev_info_from_mmc();
		break;
	}
}
